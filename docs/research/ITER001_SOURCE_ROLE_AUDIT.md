...
A qualifying incident needs all of the following on one root physical event:

1. Neutral, clocked multimodal evidence and separate commanded and realized actions.
2. At least two plausible known physical mechanisms plus explicit unknown mass, committed before
   truth and outcome access.
3. Independently established physical mechanism truth.
4. A content-bound safe diagnostic action reviewed before execution.
...
| [Paderborn bearing data](https://mb.uni-paderborn.de/en/kat/research/bearing-datacenter/data-sets-and-download) | Real and artificial fault morphology control | One bearing family and noncommercial CC BY-NC 4.0 terms; no action or recovery outcomes. |
...
| [RoboFAC](https://github.com/MINT-SJTU/RoboFAC) | Correction baseline and generated-truth leakage control | Synthetic fault truth is supplied during explanation generation. Real corrections describe visible task error, not physical mechanism. |
| [RoboFail and REFLECT](https://github.com/real-stanford/reflect) | Small multimodal incident-schema seed | Thirty real UR5e failures on one identity; real failures are teleoperated imitations and recovery authority is primarily simulator-based. |
| [Robometer](https://github.com/robometer/robometer) | Reward and progress proxy control | Public out-of-distribution data are video-only; reward and progress are not mechanism or physical outcome authority. |
| [AHA](https://github.com/NVlabs/AHA) | Procedural failure and leakage stressor | Synthetic injected modes with research-limited terms; no physical mechanism or settled recovery authority. |

The current [safe real-time active model-discrimination method](https://arxiv.org/abs/2606.19590)
is retained as an intervention baseline. It demonstrates constrained discrimination on hardware but
assumes a closed candidate set, publishes no qualifying corpus, and does not provide the integrated
open-world recovery contract.
...
2. Causal laboratory plane. A legally cleared NOS3 configuration, or Basilisk fallback, executes
   ...
   incidents.
3. Independent reality and control plane. ALFA, ESA-AD, 3W, ADAPT, DXC, RoboMIND, SAFE, and related
   sources test parsers, priors, shortcuts, reward disagreement, and distribution shift under exact
   source locks. A missing construct field cannot be borrowed from another source.

Every candidate is screened in the cheapest order: rights, immutable version, physical identity,
mechanism authority, action execution, outcome authority, clocks, then size. Large download is
forbidden when an earlier metadata gate already fails.
...
The future standalone research engine should learn the following reusable contracts from this
iteration:

- source-role assignment before download;
- exact rights decisions as typed, content-addressed evidence;
- one root-event count independent of rows, windows, branches, or annotations;
- conjunctive admission on the same scientific units;
- pre-outcome chronology and sealed truth;
- role independence by conflict group;
- separate proposal, safety, execution, and outcome authorities;
- resource accounting with unknown values distinct from zero;
- terminal blocked, rights-blocked, invalid, and infrastructure-null results; and
- generated handoff from Git-anchored state rather than chat history.
